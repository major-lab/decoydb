...
tmp_dir = "/tmp"
workdir = "/u/leongs/reproduction_projet_naim/rel20/3D/processed_26_feb_copy"
pbs_dir = "/u/leongs/reproduction_projet_naim/rel20/3D/processed_26_feb_copy/pbs"
digested_data_pk = "/u/leongs/reproduction_projet_naim/rel20/2D/digested_data.pk"
raw_dir = "/u/leongs/reproduction_projet_naim/rel20/3D/completed_decoy_26_feb"
job_name = "26_feb_copy_{index}"

def write_pbs_script(list_command, list_hairpins, index):
    curr_index = index%1 + 1
    ...
    if list_command:
        ...
        with open(os.path.join(pbs_subdir, "launch_pbs.pbs"), 'wb') as pbs_f:
            pbs_f.write(("#!/bin/bash\n"
                         ...
                         "{pbs_dir}/{name}/launch.bash 8").format(stdout_filename=os.path.join(pbs_subdir, "out.txt"),
            ...
            if line.strip():
                ...
                if os.path.exists(raw_decoy_dir):
                    ...
                    local_list_command.append(('python /u/leongs/git/various-codes/DecoyDB/3_structure_verificator.py '
                                               ...
                                               '--refine_script /u/leongs/git/various-codes/DecoyDB/minimization_scripts/new_refine.bash '
                                               '--relieve_script /u/leongs/git/various-codes/DecoyDB/minimization_scripts/new_relieve.bash '
                                               '--brushup_script /u/leongs/git/various-codes/DecoyDB/minimization_scripts/new_brushup.bash '
