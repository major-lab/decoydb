...
def get_pdb_vector_dict(pdb_file, start_5p, end_5p, start_3p, end_3p, process_name):
    list_indexes = [str(start_5p), str(end_5p), str(start_3p), str(end_3p)]
    list_a = []
    list_b = []
    list_c = []
    list_d = []
...
def process_pdb(params_dict):
    ...
    if compare_vectors(vector_dict, pdb_vector_dict, REGULAR_THRESHOLD):
        call_command("{relieve_script} {pdb_filepath}".format(relieve_script=relieve_script,
                                                              pdb_filepath=pdb_filepath),
                     echo=False)
        pdb_vector_dict = get_pdb_vector_dict(pdb_filepath, start_5p, end_5p, start_3p, end_3p, 'relieve')
        if compare_vectors(vector_dict, pdb_vector_dict, RELIEVE_THRESHOLD):
            call_command("{refine_script} {pdb_filepath}".format(refine_script=refine_script,
                                                                 pdb_filepath=pdb_filepath),
                         echo=False)
            pdb_vector_dict = get_pdb_vector_dict(pdb_filepath, start_5p, end_5p, start_3p, end_3p, 'refine')
            if compare_vectors(vector_dict, pdb_vector_dict, THRESHOLD):
                copy_to_out_dir(pdb_filepath, pdb, out_dir)
                return True
            elif compare_vectors(vector_dict, pdb_vector_dict, REFINE_THRESHOLD):
                call_command("{brushup_script} {pdb_filepath}".format(brushup_script=brushup_script,
                                                                      pdb_filepath=pdb_filepath),
                             echo=False)
